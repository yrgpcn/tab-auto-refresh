...
### Added
- 「后台保活」功能（默认开启，可在弹窗关闭）：对监控站点动态注册保活内容脚本，每 45~75 秒派发模拟鼠标/键盘活动，延缓“按用户交互计时”的服务器端会话过期；任务停止、重开、浏览器重启与开关切换时自动收敛注册
- 会话失效检测：备份 cookie 时发现会话票据从有到无（服务器端已注销），停止覆盖该站点备份，保住最后一次有效登录态，并弹系统通知提醒重新登录（按站点 6 小时节流）
- manifest 新增 `scripting` 权限（保活脚本动态注册所需）
