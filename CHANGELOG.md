...
## [tab-auto-refresh 1.7.0] - 2026-09-14
...
- manifest 新增 `scripting` 与 `idle` 权限；站点 host 权限从常驻 `<all_urls>` 改为 `optional_host_permissions`，在弹窗开始任务的用户手势里按目标 origin 申请，拒绝时任务仍可刷新、保活/关键词/备份降级不可用
