...
- 关键词监控（可选）：建任务时可填一个关键词，页面正文一出现就系统通知并自动停止任务——盯补货、盯放号、盯状态变化
- 站点权限按需申请：首次对某站点开任务时弹窗申请该站点权限，拒绝也不影响刷新本身，保活/关键词/备份才受限
...
- 角标提示：工具栏图标显示监控数量，全局暂停显示 `‖`，站点会话失效变红色 `!` 提醒重新登录
- 中英文界面：自动跟随浏览器语言
...
- **保活与心跳的前提**：机器保持开机、不休眠，浏览器保持开启。保活脚本运行在页面里、心跳由浏览器后台发出，浏览器一旦关闭都随之停止——这种"关机窗口"只能靠登录保持（cookie 备份）在重启后恢复，三者互补。
