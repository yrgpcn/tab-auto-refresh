...
- 关键词监控（可选）：建任务时可填多个关键词，用逗号分隔。页面加载后自动延迟重采样（SPA 迟渲染也能等到），命中即发系统通知。默认命中后停止任务，勾选"命中后继续监控"则持续盯守，只有新出现的关键词才再次通知。弹窗会按当前页面这条任务回填已填的关键词、"命中后继续监控"和它实际用的间隔，改它们不必先停掉再重开
