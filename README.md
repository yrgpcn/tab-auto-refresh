...
- 后台保活（默认开启）：向监控页面定期注入模拟鼠标/键盘活动，延缓"按用户交互计时"的服务器端登录过期；检测到会话已被服务器注销时暂停覆盖备份并通知，详见[使用边界](#使用边界与验证)
...
- **保活能救什么、救不了什么**：服务器端踢下线通常按三种机制之一计时。① 按“最后一次用户交互”计时——保活注入的模拟活动可以续命；② 按“最后一次 HTTP 请求”计时——只要标签页在按周期刷新就会被覆盖，无需保活，但关机期间无人刷新仍会过期；③ 绝对时长上限（如登录后固定 N 小时强制失效）——任何手段都救不了。保活只对第 ① 类有效。
- **保活的前提**：机器保持开机、不休眠，浏览器保持开启。保活脚本运行在页面里，浏览器一旦关闭，脚本随之停止——这种“关机窗口”只能靠登录保持（cookie 备份）在重启后恢复，二者互补。
- **保活可能无效的场景**：站点若校验事件的 `isTrusted`（拒绝程序合成事件），或在页面 `document.hidden` 时暂停心跳计时，注入的模拟活动不起作用。这类限制由对方服务器决定，无法绕过。
- **会话失效检测**：若已开启登录保持，插件在某次备份时发现会话 cookie 从有到无，判定服务器端会话已注销，会暂停覆盖该站点备份（保住最后一次有效登录态不被坏状态污染）并弹通知提醒重新登录；重新登录后自动恢复正常备份。
