...
test("顶层的 assets 不是数组时不当资产命中", () => {
  /* C1 之后顶层那一支压根不读 assets（见上面那条），这两条留着当脏形状守卫：
     注入体回什么形状都不许把判定带偏 */
...
/* 红→绿对照（做法见 alarm-gate.test.mjs 末尾：副本改在仓库外，仓库源码一个字不动）。
   ...
   上面第 7 处那台变异已经无从施加（2026-09-22）：C1 那次修复把顶层分支的资产判据整条删了
   （`CHANGELOG.md` 的 `[未发布]` 段），现在顶层只剩标题一条通道，没有"丢掉资产判据"可做。
   这张表是 2026-09-19 的实跑记录，不改写；顶层现在的形状由「顶层框架只按标题判墙，挑战脚本
   本身不是页面状态」那条钉住，资产列表的读侧账在 `wall-list.test.mjs` 末尾。 */
