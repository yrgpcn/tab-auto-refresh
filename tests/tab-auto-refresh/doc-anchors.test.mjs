/* 第六轮审计（`BACKLOG.md` A24）的门禁：审的是项目记忆与用户说明书对源码的漂移。
   ...
   三条判据，每条都配正反两面：
   1. 反引号里的符号名必须在仓库源码/测试/脚本里以整词还存在（改名、删函数会立刻红；
      第二十一轮之前这里是子串判据，把名字少写一个字母反而算"存在"，见下面判据 1 那段注释）
   2. 反引号里的路径必须在仓库里存在（文件或目录；插件内相对写法与裸文件名也认）
...
/* 源码语料：所有代码/配置文本拼成一份。两处刻意排除：
   ...
   - 不含本文件自己（SELF）。门禁的注释与用例里写着它盯着的那些名字，留着它等于
...
/* 判据 1 的强弱全在"整词"这两个字上（第二十一轮，`BACKLOG.md` A39）。原先是
   `corpus.includes(t)`——子串判据是单向的：少写一个字母、多写一个字母，只要结果仍落在
   ...
   现在要求 token 前后不许再接标识符字符（`A-Za-z0-9_$`）。点、连字符、冒号算分隔符，
...
test("判据不是把所有 token 都当缺失：真在用的那批逐个认得", () => {
  ...
  /* 整词判据比子串严，所以这一条要拿真语料验一面：文档里少写一个字母的那种写法，
  ...
  /* 下限从 300 降到 280（2026-09-20）：那天把 AGENTS.md 里逐轮的"第 N 轮实测 / 改前零红"叙述搬去
     `CHANGELOG.md` 和各门禁文件末尾，正文只留"改一处要连着改哪里"。搬走的那批名字里不少是门禁自己的
     内部符号，它们仍在各自的文件里，不需要项目记忆替它们点名。这一句判的是提取器或反引号写法塌了，
     不是"记忆必须复述每一个名字"，所以 280 仍然离"塌一半"有距离（改后实测 292 + 7） */
  assert.ok(AG.idents.length + RD.idents.length >= 280, "两堆符号名的总量掉下来了，提取器或文档写法变了");
});
...
test("整词才是存在：子串不算（第二十一轮补的那一半）", () => {
  /* 这一条是本轮的立论本身。子串判据漏报的方向是文档写短了：真名在语料里，
...
/* ---------- 五、对照与已知边界（实跑记录） ----------
   ...
   对照在仓库外整仓副本上跑（`_tar_ctl_r6/run.mjs`，判据吃 `TAR_DOC_ROOT`）。
   ...
   一条自我纠正：C1 第一次跑只红正向见证那一条，一般判据照绿——因为语料把本文件自己
   ...
   那一处红不出来不是覆盖面小，是判据方向的问题——`corpus.includes(t)` 只问"文档那个 token
   在语料里是不是某处的一段"，于是两个方向一起漏：文档写短了（是某个真名的一段）算存在，
   代码改名成文档名的超集（`X` → `XV2`、文档不动）也算存在。而后者正是这条门禁立起来要抓的那一类。

   7 台在仓库外整仓副本上跑 pre/post（脚本 `D:/Github/_tar_ctl_r21/ctl21.mjs`，日志
   ...
   - T3 与 T6 是同一台改名的两种落地程度，落点相反：改到"整个仓库不再以整词出现"就红，
     ...
     不是本轮新暴露的。两台一起跑是为了把这条边界的形状记下来（它以前只是推出来的）
   ...
   - T5 是本轮自己加的那处豁免的对照：`node:` 是 import 说明符前缀，源码里后面永远接着
