/* 第十七轮审计（A35）：存储这本账。审的是状态落在哪个区、键叫什么名字，与项目记忆里
   `### 存储` 那三段清单两头对账。
   ...
   `settings` 那一个键的五段，`rt-lifecycle` 管 `rtRoundKeys` 里放哪几族以及清不清得干净
   ...
   把文档 `### 存储` 里 `webhookLastResult` 那一整项删掉，同样全绿。两个方向都是安静的：
   键进错区不会崩，
   ...
   唯一的执行是把表达式体的箭头函数（`rtTab`、`rtRoundKeys` 这类）按它自己的源码 `new Function`
   ...
   与相邻门禁的分工：
   - `rt-lifecycle.test.mjs` 管"任务生命周期里这几族会不会残留"，它比的是用例自己抄的四个
   ...
   - `doc-anchors` 与 `doc-numbers` 管文档里的名字与数字对不对得上源码；键清单不在射程内
...
/* ============================ 实跑红→绿对照 ============================
   ...
   六条边界（红不到哪里，说清楚）：
   1. 文档侧只认 `### 存储` 那三行里反引号裸键名的形状，取到第一个句号为止，括号旁注整段剔掉。
   ...
   3. 只管键名与区，不管值。键放对了区、值存错了形状（该存对象存成 JSON 串）不在这根轴上；
      `settings-map` 管 `settings` 一个键的值域，其余键的值域没有账。
   4. 一次改动报几条是按集合差算的，不按调用点算：C2 那种 get 与 set 各改一处，本门禁报 2 条
