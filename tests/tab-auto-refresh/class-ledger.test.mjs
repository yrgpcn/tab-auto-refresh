...
/* ---------- 六、实测：改坏了到底谁红（红→绿对照表） ----------
   ...
   怎么读这张表：
   1. 七台里六台改前**全套零红**。这一格的账此前只有一处：`.wx-state.ok` / `.error` 两个类在
   ...
   2. **B1 只红一条而不是两条**，因为 `task-sub` 这个类名在 `popup.css` 里出现在三条规则上
      ...
      正向自然不红；红的是反向多出来的 `task-subx`。顺带说明这本账是**类名级**的，不是规则级
      （见下面边界第 3 条）
   ...
   5. B3 与 B5 是**没有第二处**的两类：往 HTML 加一个 CSS 里没有的类、往 CSS 加一条没人用的规则。
      探路实测抓到的真东西属于前一类（`wx-test`，本轮删掉）

   边界（这一根轴挡不住的）：
   1. 只认**四种写类通道**（`classList.*("…")`、`.className =/+= "…"`、`cls: "…"`、
   ...
   3. **类名级不是规则级**：账只问"这个名字有没有定义、有没有人用"，不问"这条规则写没写对"。
      ...
      只上色不声明 display）各管一面
   4. 不核**组合**：`.wx-state.ok` 只要求 `wx-state` 与 `ok` 各自有着落，两个名字同时挂在一个
