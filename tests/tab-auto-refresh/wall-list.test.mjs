/* 第二十二轮审计（`BACKLOG.md` A40）的门禁：验证墙那两条正则的**清单本身**有没有账。

   这之前只有正向账（A3 那几条用例：整页挑战要能暂停、挂件小框不许暂停、正文里有"验证码"不许
   暂停），没有反向账——九条标题特征与三条挑战域名里，**每一条各自有没有一条用例真喂它**没人管。
   ...
   五条判据，两头都要走：
   1. **正向**：清单里每一条备选都要有一条夹具**唯一命中它**（只命中这一条、不命中别的）
   2. **反向**：登记表里每一条夹具都要被清单命中，而且只能命中一条备选
      —— 这两条合起来才挡得住"改成同族另一条的拼写"（`hcaptcha` 并进 `captcha` 那种）：
      被吞的那条立刻零夹具，吞人的那条同时收到两份归属
   3. **接线**：每一条夹具都要经真实的 `decideWallFromFrames` 判成墙（顶层标题通道 + 子框架
      网址通道 + 顶层资产通道），而不是只对着正则跑——正则对了但没人把它接到判定链上，
      同样是"这条特征没人在验"
   4. **大小写**：每条带拉丁字母的特征都要有一条"只有 `/i` 才命中"的夹具，`/i` 从装饰变成账
   5. **清单内部**：一条特征的文字不许被另一条特征命中（不看夹具的独立一道，专打子串吞并）

   登记表里**只有夹具、没有特征名**：特征一律从 `WALL_TITLE_RE.source` / `CHALLENGE_URL_RE.source`
...
/* ---------- 九、实跑记录（改坏哪一处 → 红几条） ----------
   ...
   10 台在仓库外**整仓**副本上跑 pre/post（脚本 `D:/Github/_tar_ctl_r22/ctl22.mjs`，日志
   ...
   - A8/A9 的 pre 不是零，是既有覆盖里真有一条路过 `attention required` 的夹具（`frame-scan` 的
     "顶层框架判据与 A3 之前逐条一致"）和四条真吃标题 `/i` 的用例。本轮在它们之上加的是**按条归因**，
