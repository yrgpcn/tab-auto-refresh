/* 第十五轮审计（A33）：一条文案键在源码里"被取到"的那几条通道，通道的另一头没人核对。
   ...
   两头一夹夹不住的是间接通道：键名住在表里、住在属性值里，取用的时候递的是变量
   ...
   而"扫不到"在这套判据里的表现是不报错。
   反向判据刻意先剔注释再取证据：注释里写着的键名不是取用路径，把它当证据等于
...
/* 键名表的登记处。每一行都是"这张表里住的是文案键名"这一事实的声明：
   rel 是这张表的键与值之间该成立的名字关系，distinct 声明"两个槽不许用同一个值"，
...
/* ================= 实测红→绿对照（第十五轮，仓库外整仓副本 D:/Github/_tar_ctl_r15） =================
   ...
   2. 只管名字，不管内容：语言包那条文案本身长什么样、超没超微信那 20 字，
