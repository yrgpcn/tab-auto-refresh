/* 第十八轮审计（A36）：cookie 存档这本账。审的是**一份 `cookieBackup:<host>` 条目里到底有哪些
   ...
   实测（13 台变异在仓库外整仓副本各跑 pre/post 两台机器，全表记在本文件末尾）：
     - 把写侧的 `hostOnly: c.hostOnly` 改名为 `hostScope`，`validate.mjs` 与其余 541 条除了
       `doc-anchors` 那两条之外**一条不红**。那两条是"删掉本门禁"这个动作自己的副作用——
   ...
   读侧的落点是一张 `SCOPES` 表（哪个函数的哪个别名变量装着存档的哪一层）：别名**是登记的**，
   ...
   - `cookie-backup.test.mjs` 管执行器行为（查哪几层域、怎么写回、超限切尾、孤儿删不删），
     它的夹具是**本文件要核对的对象**，不是它的判据
   - `logic.test.mjs` 管 `capCookies` / `applyBackupAction` / `planBackupConvergence` 的**决策结果值**，
     不核字段名对不对得齐
   - `storage-map.test.mjs`（上一轮）管 `cookieBackup:<host>` 这个**键名**在哪个区、文档登记没有；
     本文件管键**后面那份对象**里有什么
   - `doc-numbers.test.mjs` 管 200 / 20 / 30 天这些**数字**；本文件管那条"还原三分支"的**名字与分支**
...
/* ---------- 官方字段表：抄自 developer.chrome.com 的 cookies 参考页，仓库里不再抄第二份 ----------
   ...
   `hostOnly` 在第一份里而**不在**第二份里——这正是还原那三分支存在的全部理由 */
...
/* 存档级有两条通道，都要收：
   ...
   ② `Object.assign({}, <存档别名>, { … })` 这种**就地合并**——`planBackupConvergence` 收敛
...
/* 只写不读的字段要在这里点名。`schemaVersion` 就是这一类：它是写给"将来要做迁移的那个人"的，
   今天还原侧靠 hostOnly 的真假三分支兼容 v1，没人读它。登记的意义是**反向判据不许因为它而红**，
...
/* 一段源码里所有**最外层**对象字面量的内部原文（嵌套的按配对跳过去，不重复收） */
...
/* ---------- 对照（改坏哪一处）与边界 ----------
   ...
   六条边界（红不到哪里，说清楚）：
   1. 只核**字段名在不在**，不核值。`sameSite` 递的是 `"no_restriction"` 还是 `undefined`、
      `expirationDate` 用秒还是毫秒，都不在这根轴上——那是 `logic.test.mjs` 与
      `cookie-backup.test.mjs` 拿真数据跑出来的账。
   2. `SCOPES` 是**登记的**：新增一个读存档的函数不进来登记，本文件不会自动看见它。
      挡得住"登记的那个别名没了 / 不再取用任何字段"（抽取形状红），挡不住"多一个读侧没人登记"。
   3. 官方那三份字段表（Cookie 对象 / `set` 的 details / `getAll` 的查询条件）是**手抄**的，
   ...
   4. 写侧只认 `backupCookies` 里的**一处**采集映射：`producedItemLiteral` 要求那段函数体里
   ...
   5. 夹具账认的是 "cookies 后面跟一个左方括号" 这个形状。夹具改叫别的键名就不在账上了，
      ...
      下**全部** `.test.mjs`，含本文件，而共用的 `stripComments` 只按行认出注释的起始行
