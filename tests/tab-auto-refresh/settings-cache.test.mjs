/* 2.1.0 批次 1 的门禁：生效设置的内存快照必须在正确时机作废。
   ...
   桩件的 set 不自动回流 onChanged（真实 Chrome 会），所以"写完立刻读"测的是写盘后那次显式
   失效，事件回流另开一条用例，两者不互相顶包。
   每条 onChanged 的改动键一律挑 bypassCache：它不在后台"需要收敛"的名单里，不会牵出
   reconcileKeepAlive 去多读一轮设置、多注入一次脚本，把断言的因果搅浑。
   每个用例一套新桩件 + 带查询串的 import：监听器数组跨实例累积会让断言假绿。 */

import assert from "node:assert/strict";
import test from "node:test";
import { pathToFileURL } from "node:url";

/* 红→绿对照用：TAR_BG 指到另一份 background.js 的绝对路径（该文件必须和自己的
   shared/ 目录同级，background.js 是按相对路径 import 的）。CI 上不设这个变量。
   对照做法见文件末尾的注释 */
...
function makeEnv() {
  ...
  const calls = { syncGet: 0, localGet: 0, executeScript: 0 };
  ...
  const area = (name) => ({
    async get(keys) {
      ...
      const out = {};
      for (const k of Object.keys(keys)) out[k] = k in s ? s[k] : keys[k];
      return out;
    },
    async set(obj) {
      Object.assign(store[name], JSON.parse(JSON.stringify(obj)));
    },
    async remove(keys) {
  ...
  const chrome = {
    ...
    alarms: { create: noop, clear: noop, getAll: async () => [], onAlarm: { addListener: noop } },
    tabs: {
      get: async () => null,
      update: noop,
      create: async () => ({ id: 1 }),
    ...
    cookies: { getAll: async () => [], set: noop },
    power: { requestKeepAwake: noop, releaseKeepAwake: noop },
    i18n: { getMessage: () => "" },
  };

  /* 走真实的 onMessage 入口。keepalive-query 只读一次设置并把两个注入开关回出来，
     正好当"读侧探针"：它反映的是后台此刻生效的值，不是弹窗自己合并的值 */
  const send = (msg) =>
    new Promise((resolve) => {
      listeners.message.at(-1)(msg, {}, resolve);
    });

  /* 喂一次 onChanged，模拟另一台设备同步过来的改动。
     changes.settings 必须存在，否则监听器在入口就早退；但两侧的差值一律只落在 bypassCache
     上，后台收敛名单里的那几个键保持相等——不然会牵出 reconcileKeepAlive 多读一轮设置、
     多注入一次脚本，把"这条断言验的是失效"这个因果搅浑。
     真实生效值由 getSettings 回读 store 得到，与这份 payload 无关 */
  const base = {
    keepAlive: true,
    httpHeartbeat: true,
    skipOnActivity: true,
  ...
  const externalChange = async () => {
    await listeners.changed.at(-1)(
      {
        bypassCache: { oldValue: true, newValue: false },
        settings: {
          oldValue: base,
          newValue: Object.assign({}, base, { bypassCache: false })
        }
      },
      "sync"
    );
  ...
  return { chrome, store, calls, send, externalChange };
}

async function boot(env) {
  globalThis.chrome = env.chrome;
  /* 加查询串强制每次 import 都新实例化一个模块（等价于 SW 回收后重启） */
  await import(BG + "?seq=" + ++importSeq);
}

test("settings are read from disk once, then served from the snapshot", async () => {
  const env = makeEnv();
  env.store.sync.settings = { keepAlive: false, bypassCache: true };
  await boot(env);

  const first = await env.send({ type: "keepalive-query" });
  ...
  const second = await env.send({ type: "keepalive-query" });
...
test("save-settings invalidates before merging and again after writing", async () => {
  const env = makeEnv();
  env.store.sync.settings = { keepAlive: true, httpHeartbeat: true };
  await boot(env);
  await env.send({ type: "keepalive-query" }); /* 预热：快照里 keepAlive=true */
  ...
  const after = await env.send({ type: "keepalive-query" });
...
test("an external settings change invalidates the snapshot", async () => {
  const env = makeEnv();
  env.store.sync.settings = { keepAlive: true };
  await boot(env);
  const primed = await env.send({ type: "keepalive-query" });
  assert.equal(primed.heartbeat, true);

  env.store.sync.settings = { keepAlive: false };
  await env.externalChange();
...
test("a read in flight across an invalidation must not refill the snapshot", async () => {
  ...
  const pending = env.send({ type: "keepalive-query" }); /* 此刻已取到 keepAlive=true，挂起中 */
  env.store.sync.settings = { keepAlive: false };
  await env.externalChange(); /* 失效发生在这次读盘途中 */
  release();
