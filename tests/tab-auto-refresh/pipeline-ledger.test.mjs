/* 第二十五轮审计（`BACKLOG.md` A43）的门禁：`.github/workflows/` 那两份文件是仓库里唯一
   ...
   1. 测试 glob：`.github/workflows/ci.yml` ↔ `release.yml` ↔ 根 `package.json` 的 test 脚本，
      三处写的是同一串，而**这一串必须真匹配到仓库里的每一个 `.test.mjs`**
   ...
   YAML 一律**按行形状切**，不写解析器：这里要量的是某一行的字面量，不是嵌套结构。
...
/* 步骤次序账：角色按**运行内容**认，不按步骤名（名字是给人看的，漂了就红在别处） */
const ROLES = [
...
/* ---------- 七、实测：改坏了到底谁红（红→绿对照表） ----------
   ...
   - 每台 post 多带的那条**常驻对照**红是预期的：那三条对照的参照物就是被改的那处真值本身，
     它们跟着红恰好证明对照不是空跑。
   ...
   - 名字账的十一个落点里，实测真各改过其中一处的只有六个（tag 模式、清理前缀、剥前缀、`--prefix`、
     ...
     没测的就是没测的。
   - 还有一处**本文件自己的坑**记在这里：这张表写在块注释里，而 glob 那种写法是"两个星号紧跟一个斜杠"，
     连起来正好是块注释的终止符——把那一串原样抄进表格，本文件的注释当场被截断，
     `scripts/validate.mjs` 的语法检查红一条（本轮第一次跑到就是这个症状）。所以 B3 那一格写中文描述。

   边界（刻意不覆盖的）：

   1. 只核 workflow 与文档**写的字面量**，不核 GitHub 那个托管服务**怎么做**：`actions/checkout@v4`
