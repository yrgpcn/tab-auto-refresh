...
const ROWS = [
  ...
  /* 掉线与保活 */
  {
    id: "掉线确认次数",
    docs: [AGENTS, README],
    re: /连续\s*(\d+)\s*次才冻结|计入同一个\s*(\d+)\s*次确认窗口|各要连续\s*([一二两三四五六七八九十\d]+)\s*次采样/g,
    want: (v) => [S(v.LOST_SAMPLES)],
    norm: (caps) => [S(cnNum(caps.find((c) => c !== "")))]
  },
  {
    id: "掉线通知节流（小时）",
    docs: [AGENTS, README],
    re: /按\s*(\d+)\s*小时节流通知|同一个\s*(\d+)\s*小时窗口/g,
  ...
  /* 关键词与检测链 */
  {
    id: "关键词单条字数与条数上限",
    docs: [AGENTS, README],
    re: /每条\s*≤(\d+)\s*字、上限\s*(\d+)\s*个|最多\s*(\d+)\s*个、每个最长\s*(\d+)\s*字/g,
    want: (v) => [S(v.KW_MAX_LEN), S(v.KW_MAX_COUNT)],
    /* AGENTS 先写字数、README 先写条数，两种语序都得归到 want 的顺序上，
       否则改一处措辞就红成"数字不符"，指的是空气 */
    norm: (caps) => (caps[0] !== "" ? [caps[0], caps[1]] : [caps[3], caps[2]])
  },
  {
    id: "错误页连击次数（README 的人话写法）",
    docs: [README],
    re: /连续\s*([一二两三四五六七八九十\d]+)\s*次心跳/g,
    want: (v) => [S(v.PAUSE_SAMPLES)],
    norm: (caps) => [S(cnNum(caps[0]))]
  },
  {
    id: "验证墙连击次数（README 的人话写法）",
    docs: [README],
    re: /连续\s*([一二两三四五六七八九十\d]+)\s*次加载/g,
    want: (v) => [S(v.CAPTCHA_SAMPLES)],
    norm: (caps) => [S(cnNum(caps[0]))]
  },
  {
...
/* ---------- 六、实跑记录与已知边界 ----------
   ...
   第二批（2026-09-21，README 功能点描述与代码对齐那一轮新写进去的数字；副本
   `D:/Github/_tar_ctl_doc/control.mjs`，base 全绿，此时 47 条）：

   E1  README「各要连续两次采样」→「三次」……… 只红「对账 11：掉线确认次数」（文档写 3，源码现推 2）
   E2  README「同一个 6 小时窗口」→「5 小时」…… 只红「对账 12：掉线通知节流（小时）」
   E3  AGENTS.md「按 6 小时节流通知」→「5 小时」… 同一行红，报的是 AGENTS.md 那一头。
       E2/E3 是一对：这一行挂着两个 docs，只钉得住一头不算覆盖
   E4  README「最多 10 个」→「12 个」……………… 只红「对账 17：关键词单条字数与条数上限」，
       报 "文档写 100 / 12" —— 那行 norm 按来源分支换捕获组顺序，改坏一头不会被换序绕过去
   E5  AGENTS.md「上限 10 个」→「11 个」………… 同一行红，AGENTS.md 那一头
   E6  README「连续两次心跳」→「三次」…………… 只红「对账 18：错误页连击次数（README 的人话写法）」
   E7  README「连续三次加载探到」→「四次」……… 只红「对账 19：验证墙连击次数（README 的人话写法）」
   E8  README 那句改成「各要采样一次以上」（数字删掉）… 红 2 条：对账 11 与"判据覆盖面"

   这一批同样是"改坏一处只红一行"，没有连带拖下别的行。本轮自己返工过两次：
   - 对账 12 第一版漏了 `norm`。两个捕获组的交替式，空的哪一组留在 caps 里，于是它绿的时候
     打印 "文档写 6 / "、红的时候也报不出真正的差异 —— 交替式里有必空的捕获组就必须配 `norm`，
     这一行的写法与对账 11 同形（取非空那一组）
   - 上面 D1~D10 里的"对账 N"是当时那一轮的序号，ROWS 加行会让序号整体后移。
     认行认 id，不认 N
