...
- **组织约定**：报告按轮次编号（`01` 首轮 … `13` 扩展直连微信可行性实测、`14` 微信直连集成批次、`15` 12 报告核对与修复 + 教程页、`16` 教程页配图 + VPS 中继下线、`17` 微信卡片 20 字上限与消息重排），每轮同步更新 `_code-review/README.md` 索引；引用结论写 `函数名 @ <commit>:<行>` 而非裸行号——行号每轮都在漂（实证：`checkKeyword` 从 `0d9fa8a:637` 漂到 `0b04251:640`）
- **回归脚本**（改动对应功能后必跑）：
