...
- **组织约定**：报告按轮次编号（`01` 首轮 … `11` 弹窗尺寸与默认值批次），每轮同步更新 `_code-review/README.md` 索引；引用结论写 `函数名 @ <commit>:<行>` 而非裸行号——行号每轮都在漂（实证：`checkKeyword` 从 `0d9fa8a:637` 漂到 `0b04251:640`）
- **回归脚本**（改动对应功能后必跑）：
