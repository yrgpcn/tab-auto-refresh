...
- 审计的"A 系列"到第二十五轮（A43）清账，2026-09-20 起收口：**默认不再自开新一轮**。第二十一到二十五轮
  五轮改的产品代码是零、补的全是门禁，而上一回抓到真 bug 是删掉一个死类名那一轮。剩下的三件事都要人在场：
  V1 只能真机验、E2 是"本地审核归档要不要入库"的取舍、发不发版是对外动作。下一轮真要开，
  先答得出"这一轮打算改哪一行产品代码"；答不出来就不开。队列与理由见 `BACKLOG.md`
...
- class 名字三本账要一起对（第二十三轮 A41）：**CSS 定义了哪些类** ↔ **HTML 用到位的是哪些** ↔ **JS 运行时写进去的是哪些**（四种写法都算：`classList` 调用、`className` 赋值、拼接用的 `cls` 字面量、`setAttribute("class", …)`）。判据在 `class-ledger.test.mjs`：正向挡"挂了没样式"，反向挡"改一头忘另一头"，页面清单从插件目录现推（每个 `.css` 要有同名 `.html` 与 `.js`，且那页真的 `<link>` 引它），新增一个带样式的页面要在那里登记一组条数下限。这一格此前唯一的账是 `popup-repopulate.test.mjs` 那两个颜色类，`popup.css` 43 个类名里其余四十一条判据都没有（七台仓库外变异里六台改前全套零红）。**类改名要一次改齐 CSS、HTML、JS 三处**；往 HTML 加一个 CSS 里没有的类与往 CSS 加一条没人用的规则，两头各红一头。刻意不覆盖：类名级不是规则级（整条规则删掉而类名还在别处出现，全绿）、组合不在账上、属性值对不对不在账上（高度由 `--measure` 管，见验证清单第 4 条）
