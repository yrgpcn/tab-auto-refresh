...
- 偏好设置存于 `chrome.storage.sync`：`settings` 为 `{ bypassCache, skipDiscarded, cookieBackup, keepAlive, httpHeartbeat, skipOnActivity, keepAwake, captchaGuard, webhookUrl, webhookEvents, lastIntervalSec }`；默认值统一在 `shared/config.js` 的 `DEFAULT_SETTINGS`（弹窗与后台共用）；读取时若 sync 为空会尝试从 local 迁移旧设置
...
- **Webhook**（默认空=关）：`postWebhook(event, payload)` 过 `normalizeWebhookUrl`（仅 http/s）+ 事件白名单；载荷并填 `content`(Discord) / `text`(Slack、Telegram) / `body`(冗余兜底，**无服务认它**) + `type`/`url`/`host`/`ts`；fetch 在 await 链路内 + 15s AbortController（防 SW 回收丢请求）；失败静默。事件源：掉线确认、关键词命中、`stopTaskWithNotice`（先取 url 再停——08 §3.6）、自动暂停。**四处调用点必须 await**（09 复审 §3：首版全写成 `void`，与函数注释自相矛盾，实测处理器会先于 webhook 结束）。**别再说"ntfy 开箱即用"**：实测 ntfy 只在根端点解析 JSON，POST 到 `/<主题>` 会把整个 JSON 当正文存下，且载荷无 `topic` 字段、无法改填根端点（会 400）——已把 README/popup 提示/代码注释统一为"能送达，但正文是原始 JSON"（除非将来给载荷补 `message` 字段）。地址非法时 `postWebhook` 静默返回，故弹窗另有 `renderWebhookValidity()` 内联提示（`webhookInvalid`）
...
- **组织约定**：报告按轮次编号（`01` 首轮 … `10` 档 1 修缮），每轮同步更新 `_code-review/README.md` 索引；引用结论写 `函数名 @ <commit>:<行>` 而非裸行号——行号每轮都在漂（实证：`checkKeyword` 从 `0d9fa8a:637` 漂到 `0b04251:640`）
- **回归脚本**（改动对应功能后必跑）：
