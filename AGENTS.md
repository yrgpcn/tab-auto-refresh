...
- 偏好设置存于 `chrome.storage.sync`：`settings` 为 `{ bypassCache, skipDiscarded, cookieBackup, keepAlive, httpHeartbeat, skipOnActivity, keepAwake, webhookUrl, webhookEvents, lastIntervalSec }`；默认值统一在 `shared/config.js` 的 `DEFAULT_SETTINGS`（弹窗与后台共用）；读取时若 sync 为空会尝试从 local 迁移旧设置
...
- **异常自动暂停**（06 §4.3）：心跳侧 5xx/404 连续 2 次（`rt:error:<tabId>`）→ `autoPaused:{reason:"error-page"}`，恢复 2xx 自动解除；页面侧 `probeCaptcha`（标题/正文特征 + CF/reCAPTCHA 资源 iframe，连续 2 次命中）→ `autoPaused:{reason:"captcha"}`，过墙后弹窗"恢复"按钮解除。alarm 照常续跑、onAlarm 见 `autoPaused` 早退——暂停零重建；**独立计数不进 sessionProbe**，不污染备份冻结语义；角标 `⚠` + 通知 + webhook `task-paused` 全链路可解释（08 §3.5 要求同批落实）。onAlarm 里 **tab 存在性检查排在 autoPaused 之前**：被自动暂停的任务若标签页被关掉，仍按老行为"停任务 + 通知"，不留僵尸
...
- **Webhook**（默认空=关）：`postWebhook(event, payload)` 过 `normalizeWebhookUrl`（仅 http/s）+ 事件白名单；载荷并填 `content`(Discord/Slack)/`text`(Telegram)/`body`(通用) + `type`/`url`/`host`/`ts`；fetch 在 await 链路内 + 15s AbortController（防 SW 回收丢请求）；失败静默。事件源：掉线确认、关键词命中、`stopTaskWithNotice`（先取 url 再停——08 §3.6）、自动暂停。**四处调用点必须 await**（09 复审 §3：首版全写成 `void`，与函数注释自相矛盾，实测处理器会先于 webhook 结束）
