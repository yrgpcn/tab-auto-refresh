...
- **1.8.0 已完成开发（manifest 版本号 `1.8.0`，未打 tag、未推送）**：新增**微信直连通知**——扩展 SW 直接调腾讯官方接口推模板消息，不经中继、不经第三方服务商；新增**微信配置教程页** `wechat-setup.html`（弹窗配置页有「配置教程」直达，教程文件本身就在仓库里）；事件清单键名 `webhookEvents` → `notifyEvents`（旧值自动接续）；修 `[hidden]` 被作者样式 `display` 压过的通用缺陷；并接手 **12 号独立复审**（基线 `4fad900`）核对出的 6 处缺陷（§1 空事件集合反而全发已在上一批修掉，§2/§3/§4/§5/§6/§7 本轮修完——逐条裁定与证据见 `_code-review/15-12报告核对与修复.md`）。要点见下节「1.8.0 实现要点」
...
- **凭据四项**（`wechatEnabled` / `wechatAppId` / `wechatAppSecret` / `wechatOpenId` / `wechatTemplateId`）：默认全空 + 开关关闭。存 `chrome.storage.sync`——用户已知情并接受同步到自己的 Google 账号，README 安全说明里如实披露。模板变量名**必须**是 `title` 与 `content`（对应后台 `{{title.DATA}}` / `{{content.DATA}}`），换名字会推出一张空白卡片；这条写进了配置页常驻提示
...
- **教程页 `wechat-setup.html`（+`wechat-setup.js`/`.css`）**：既是仓库里的那份教程，也是弹窗「配置教程」指向的页面——**同一份文件**，不存在"文档站与包内页面各自漂移"。文案全部走语言包（与弹窗同一套 `[data-i18n]` 注入），页面只有结构与键名；`_smoke_extension_boot.mjs` 会真开这个页面，断言五个小节齐全、`data-i18n` 全部注入（不残留键名）、外部 css 未被 CSP 拦、测试号链接指向官方域名
...
- **组织约定**：报告按轮次编号（`01` 首轮 … `13` 扩展直连微信可行性实测、`14` 微信直连集成批次、`15` 12 报告核对与修复 + 教程页），每轮同步更新 `_code-review/README.md` 索引；引用结论写 `函数名 @ <commit>:<行>` 而非裸行号——行号每轮都在漂（实证：`checkKeyword` 从 `0d9fa8a:637` 漂到 `0b04251:640`）
- **回归脚本**（改动对应功能后必跑）：
