...
| 编号 | 优先级 | 一句话 | 状态 |
| --- | --- | --- | --- |
| A21 | P3 | 停任务时在飞的关键词检测链零门禁：删掉 `stopTask` 里的 `detectChains.delete(tabId)`，全套（含 A20 新增那 18 条）一条都不红 | 待做 |
...
### A21 停任务时在飞的关键词检测链零门禁

- 位置：`stopTask` → `detectChains.delete(tabId)`（终止该页在飞的那条检测链）
- 症状：A20 对照里的反向那处顺手量到的。把这一行删掉，`detect-chain.test.mjs` 与本文件全绿。
  真实后果要这样才出现：链正卡在 `executeInAllFrames` 里没回来时用户点了停止。放开之后第 777 行那道
  token 判据认"我还是当前那条"（`detectChains.get(tabId) !== token` 之所以不成立，正是因为停任务时
  没人把它删掉），于是链按**它在 await 之前读好的那份 task**（旧关键词）继续判命中 →
  用户已经点了停止，通知中心却弹出一条"命中"（默认还会再走一遍 `stopTask`）。
  `onHit: "continue"` 时更实在一点：第 790 行那段回读到 `tasks[tabId]` 就写 `notifiedKeys`，
  若这期间用户在**同一张标签页**上以新关键词重开了任务，写回去的就是新任务的键，
  等于用上一轮关键词的在场集压制新一轮的首次通知
- 为什么测不出来：`detectChains` 是模块级 Map，每个用例 import 一份新实例，上一例的残留不会串；
  而链的三次采样是 `[0, 3000, 10000]` 真 `setTimeout`，现有用例只排 50 毫秒宏任务，
  永远停在"第一次采样刚发出"那一步，没人把它挂到"注入还没回来时任务被停"这个交错上
- 改法方向（A20 实跑时确认这条路走得通，不需要假时钟）：把 `executeScript` 换成"带 `args` 的那一类
  先 await 一个由用例掌握的 promise、放开时回一份命中结果"，序列是
  `tabUpdated` 起链 →（链卡在注入里）→ `send({type:"stop"})` → 放开 → 排宏任务 →
  断言这一轮**没有** `keyword-hit` 通知、也没有第二次注入。先要有一条正向见证（不停任务时同样形状
  必须发出通知），否则整条用例可以靠"链压根没跑到"变绿。归到 `detect-chain.test.mjs`，
  与 A20 的"先证明通道跑到了、再断言这条路径不该跑"是同一条纪律
