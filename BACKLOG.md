...
又跑了第四、第五轮审计（第四轮审校验闸门、第五轮审 README 与截图，都不是插件本体），账记在下面两段
A22 与 A23。三段之后剩下的只有待定项 E2，
以及只能真机验的 V1（工程账 E3 已在同日结案，见下面"工程账"那一节）。

A22 同日结案，**插件产品代码一个字没改**——这一轮审的不是插件，是当时全仓库仅剩的两面零门禁之一：
`ci.yml` 与 `release.yml` 每次都用 `scripts/validate.mjs` 当闸门，而闸门自己没有任何测试。
（另一面是 `README.md` 与两张截图，紧跟着的 A23 就是它。）
...
- 三条文案漂移，每条都是先读源码确认行为、再改话术：
  ...
  而 `probeCaptcha` 在每次页面加载完成探不到墙时就 `resumeTaskAuto`（`background.js:1817-1819`），
  这条自动解除本来就有正向用例（`detect-chain.test.mjs:205`）和删掉它必红的对照。
