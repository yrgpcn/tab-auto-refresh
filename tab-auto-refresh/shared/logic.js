...
/* 验证墙的标题特征。判据面刻意只取标题与挑战域名资产，不扫正文：正文里"验证码"
   ...
   这两条正则（上面的 `CHALLENGE_URL_RE` 与本条）的每一条备选都要有一条真喂它的夹具，
...
/* webhook 回执分类（纯函数，纪律 1）：输入是已经取到的 HTTP 状态与响应正文，
   ...
   刻意不去认 Slack 的纯文本错误（"no_service" 那一类）：正文来自第三方，
...
/* 外发载荷里的网址：同样只留 origin+pathname，把 query 与 hash 剪掉。
   形状与 urlKey 相同却故意不共用一个函数——urlKey 是页面认领的匹配键，为了匹配
...
/* 角标那五态集中在这里判（纯函数，纪律 1）。输入全是上面 `aggregateBadgeFacts` 现推出来的事实，
   不做任何浏览器调用。五支的先后次序不在这里逐条重抄（那份清单住在 `AGENTS.md` 的
   ...
   颜色那条链四支、文字那条链五支，这个不对称不是笔误：文字要 `count > 0` 才给 `‖`，
